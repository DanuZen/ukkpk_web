...
export const HomeSlideshow = () => {
  ...
  if (images.length === 0) {
    return <section className="relative w-full h-screen overflow-hidden bg-gradient-to-br from-primary/20 via-background to-secondary/20 -mt-16">
  ...
  return <section className="relative w-full h-screen overflow-hidden group -mt-16">
      ...
      <div className="relative w-full h-full">
        {images.map((image, index) => <div key={image.id} className={`absolute inset-0 transition-opacity duration-1000 ${index === currentIndex ? 'opacity-100' : 'opacity-0'}`}>
            <img src={image.image_url} alt={`Slide ${index + 1}`} className="w-full h-full object-cover" />
        ...
        opacity: opacity
          ...
          animationDelay: '0.4s'
            ...
            <Link to="/radio" className="flex-1">
              <Button size="sm" variant="outline" className="w-full text-xs sm:text-base px-4 sm:px-6 py-2 sm:py-5 bg-white/10 backdrop-blur-sm border-white/30 hover:bg-white/20 text-white shadow-lg hover:shadow-xl transition-all">
                Dengar Radio
              </Button>
            </Link>
          </div>
