...
const ProfilUkkpk = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const [membersRes, profileRes, strukturRes] = await Promise.all([
          supabase.from('organization').select('*').order('order_index', { ascending: true }),
          supabase.from('profile_settings').select('*').limit(1).maybeSingle(),
          supabase.from('struktur_organisasi').select('*').order('created_at', { ascending: true }),
        ]);

        if (membersRes.data) setMembers(membersRes.data);
        if (profileRes.data) setProfile(profileRes.data);
        if (strukturRes.data) setStrukturData(strukturRes.data);
      } catch (error) {
              ...
              <div className="max-w-3xl mx-auto text-center space-y-4 sm:space-y-6">
                <div className="flex justify-center mb-4 md:mb-6">
                  <div className="inline-flex items-center gap-2 sm:gap-3 bg-white/10 backdrop-blur-sm border border-white/20 text-white px-3 py-1.5 sm:px-5 sm:py-2.5 rounded-full">
                    ...
                    <span className="text-xs sm:text-sm font-medium">Unit Kegiatan Kampus UNP</span>
                  </div>
                </div>

                <div className="overflow-hidden inline-block max-w-full">
                  <h1 className="text-4xl sm:text-5xl md:text-6xl lg:text-7xl font-bold text-white drop-shadow-2xl typewriter-text-profil whitespace-nowrap">UKKPK UNP</h1>
                </div>
                <style>{`
                  .typewriter-text-profil {
                    ...
                    animation: typing-profil 2s steps(9) 0.5s forwards, blink-profil 0.75s step-end 0.5s 4, remove-border-profil 0.01s 3.5s forwards;
                  }
                ...
                <Card className="bg-white/60 border-primary/20 shadow-2xl overflow-hidden">
                  <CardContent className="pt-8 pb-8 px-4 sm:pt-10 sm:pb-10 sm:px-8 md:pt-12 md:pb-12 md:px-14 relative">
                    ...
                    <div className="relative z-10">
                      <p className="text-xs sm:text-base md:text-lg text-muted-foreground leading-relaxed text-justify">
            ...
            <div className="max-w-6xl mx-auto">
              <AnimatedSection animation="bounce-in" delay={100}>
                <div className="text-center mb-12">
                  ...
                  <p className="text-xs sm:text-base md:text-lg text-muted-foreground max-w-3xl mx-auto mb-6">Perjalanan panjang UKKPK dari masa ke masa dalam mengembangkan komunikasi dan penyiaran kampus</p>
                  <div className="w-20 h-1 bg-gradient-to-r from-transparent via-primary to-transparent mx-auto" />
                </div>
              </AnimatedSection>
                  ...
                  <AnimatedSection key={index} animation="flip-up" delay={item.delay}>
                    <Card className="bg-white/60 border-primary/20 shadow-xl overflow-hidden transition-all duration-500 h-full">
                      <CardContent className="pt-8 pb-8 px-4 sm:pt-10 sm:pb-10 sm:px-8 md:pt-10 md:pb-10 md:px-10 relative flex flex-col h-full">
                        ...
                        <p className="text-xs sm:text-sm md:text-base lg:text-lg text-muted-foreground leading-relaxed text-justify">{item.content}</p>
                      </CardContent>
                    </Card>
                  </AnimatedSection>
                ))}
          ...
          <div className="container mx-auto px-2 sm:px-6 relative z-10">
            <AnimatedSection animation="zoom-in">
              <div className="text-center mb-12">
                ...
                <p className="text-xs sm:text-base md:text-lg text-muted-foreground max-w-3xl mx-auto mb-6">Berbagai bidang kegiatan yang menjadi wadah pengembangan skill dan bakat mahasiswa di dunia komunikasi</p>
                <div className="w-20 h-1 bg-gradient-to-r from-transparent via-primary to-transparent mx-auto" />
              </div>
            </AnimatedSection>
                  ...
                  <AnimatedSection key={index} animation="flip-up" delay={100 + index * 100}>
                    <Card className="group transition-all duration-300 hover:-translate-y-1 shadow-lg h-full bg-white/60">
                      <CardContent className="pt-8 pb-8 px-4 sm:px-6 md:px-8 text-center flex flex-col items-center relative h-full">
                        <div className="absolute top-0 right-0 w-20 h-20 bg-gradient-to-br from-primary/5 to-transparent rounded-bl-[100px]" />
                        <div className="absolute bottom-0 left-0 w-20 h-20 bg-gradient-to-tr from-secondary/5 to-transparent rounded-tr-[100px]" />
                        <div className="inline-flex p-3 sm:p-4 rounded-full bg-primary/10 text-primary mb-4 group-hover:scale-110 transition-transform relative z-10">{feature.icon}</div>
                        <h3 className="text-base sm:text-lg md:text-xl font-semibold mb-3 min-h-[3.5rem] flex items-center justify-center relative z-10">{feature.title}</h3>
                        <p className="text-xs sm:text-sm md:text-base text-muted-foreground text-center leading-relaxed relative z-10">{feature.description}</p>
                      </CardContent>
                    </Card>
                  </AnimatedSection>
                ))}
          ...
          <div className="container mx-auto px-2 sm:px-6 relative z-10">
            <AnimatedSection animation="zoom-in">
              <div className="text-center mb-12">
                ...
                <p className="text-xs sm:text-base md:text-lg text-muted-foreground max-w-3xl mx-auto mb-6">Prinsip dan nilai-nilai yang menjadi landasan dalam setiap kegiatan dan karya UKKPK</p>
                <div className="w-20 h-1 bg-gradient-to-r from-transparent via-primary to-transparent mx-auto" />
              </div>
            </AnimatedSection>
                ...
                >
                  <Card className={`text-center hover:shadow-lg transition-all duration-300 hover:-translate-y-1 shadow-lg h-full bg-white/60 ${index === 2 ? "w-[calc(50%-0.375rem)] sm:w-[calc(50%-0.75rem)] md:w-full" : ""}`}>
                    <CardContent className="pt-8 pb-8 px-4 sm:px-6 md:px-8 relative flex flex-col items-center h-full">
                      ...
                      <p className="text-xs sm:text-sm md:text-base text-muted-foreground text-center leading-relaxed relative z-10">{value.description}</p>
                    </CardContent>
                  </Card>
                </AnimatedSection>
              ))}
          ...
          <div className="container mx-auto px-2 sm:px-6 relative z-10">
            <AnimatedSection animation="bounce-in">
              <div className="text-center mb-12">
                ...
                <p className="text-xs sm:text-base md:text-lg text-muted-foreground max-w-3xl mx-auto mb-6">Kenali tim pengurus dan struktur organisasi UKKPK yang solid dan profesional</p>
                <div className="w-20 h-1 bg-gradient-to-r from-transparent via-primary to-transparent mx-auto" />
              </div>
            </AnimatedSection>
                  ...
                  <AnimatedSection key={member.id} animation="flip-up" delay={100 + (index % 3) * 100}>
                    <Card className="overflow-hidden hover:shadow-lg transition-all duration-300 hover:-translate-y-1 h-full bg-white/60">
                      <CardContent className="p-2 sm:p-4 md:p-6 text-center">
                        ...
                        <h3 className="text-xs sm:text-base md:text-lg font-semibold mb-1">{member.name}</h3>
