...
const BeritaDetail = () => {
  ...
  const incrementViewCount = async () => {
    try {
      const { data: currentData } = await supabase.from('news').select('view_count').eq('id', id).single();
      if (currentData) {
        await supabase.from('news').update({ view_count: (currentData.view_count || 0) + 1 }).eq('id', id);
      }
    } catch (error) {
  ...
  const checkIfLiked = async () => {
    ...
    try {
      ...
      const { data } = await supabase.from('content_likes').select('id').eq('content_id', news.id).eq('content_type', 'news').eq('user_identifier', userIdentifier).maybeSingle();
      setHasLiked(!!data);
    } catch (error) {
  ...
  const handleLike = async () => {
    ...
    try {
      ...
      if (hasLiked) {
        ...
        await supabase.from('news').update({ likes_count: Math.max(0, news.likes_count - 1) }).eq('id', news.id);
        setNews({ ...news, likes_count: Math.max(0, news.likes_count - 1) });
        setHasLiked(false);
      } else {
        await supabase.from('content_likes').insert({ content_id: news.id, content_type: 'news', user_identifier: userIdentifier });
        await supabase.from('news').update({ likes_count: news.likes_count + 1 }).eq('id', news.id);
              ...
              {otherNews.length > 0 && (
                <div className="mt-6 md:mt-12 pt-4 md:pt-8 border-t border-border">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-3 md:gap-6">
                    {otherNews.map(item => (
                      <div key={item.id} className="group cursor-pointer bg-card rounded-md md:rounded-lg overflow-hidden border border-border hover:shadow-lg transition-all duration-300" onClick={() => { navigate(`/berita/${item.id}`); window.scrollTo(0, 0); }}>
              ...
              <div>
                ...
                <div className="space-y-6">
                  {relatedNews.map((relatedItem, index) => (
                    <div key={relatedItem.id} className={`group cursor-pointer ${index !== relatedNews.length - 1 ? 'border-b border-border pb-6' : ''}`} onClick={() => { navigate(`/berita/${relatedItem.id}`); window.scrollTo(0, 0); }}>
                      <div className="flex items-center gap-2 text-sm text-muted-foreground mb-2">
                        <Calendar className="h-4 w-4" />
                        <span>{new Date(relatedItem.published_at || relatedItem.created_at).toLocaleDateString('id-ID', { weekday: 'long', day: 'numeric', month: 'long', year: 'numeric' })}</span>
                      </div>
